...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File $runner `
    -ProjectRoot "C:\path\to\UnityProject" `
    -ScenarioBundlePath "C:\external\example-scenario"
~~~
...
| Unity | Test Framework | 상태 |
| --- | --- | --- |
| `2022.3.62f3` | `1.1.33` | `APPROVED` |
| `6000.0.69f1` | `1.6.0` | `APPROVED` |
| `6000.5.3f1` | `1.7.0` | `APPROVED` |
...
`-nographics`, `-quit`, `-runSynchronously`, `-executeMethod`, `-accept-apiupdate`, `-ignorecompilererrors`는 금지한다. 프로세스는 suspended 상태에서 kill-on-close Job Object에 먼저 연결한 뒤 재개하며, timeout과 자식 프로세스 잔류를 fail-closed로 판정한다.
...
정상 stdout은 schema `1.0.0` JSON 문서 하나뿐이며, 동일한 문서를 외부 `result.json`에 저장한다. 결과에는 Doctor, compatibility, Unity trust, selection, process control, isolation, artifacts, NUnit, Editor.log, scenario/captures, 원본 및 Git 무결성, verification scopes, warnings, failures, blockers와 evidence가 있다.
...
실제 승인 suite는 개발자용이며 CI에서 실행하지 않는다. 최종 18-case 승인 결과는 [Unity Play Verification 실제 Unity 승인 기록](../validation/unity-play-verification-real-unity-acceptance.md)에 있다.
